...
my( $first, $second, $file, $outFasta, $lower, $upper ) = &parsecom( \%opts );
...
my @kept; # array to hold only fragments within size selection range

# output file name
my $out = $first . '_by_' . $second . '_' . $lower . '_' . $upper . '.log';
...
my $fragCounter = 0;
foreach my $frag( @kept ){
	$fragCounter++;
	print OUT ">fragment_$fragCounter\n";
	print OUT $frag, "\n";
}
close OUT;
...
sub help{
  ...
  print "\t-o:\tUse this to specify the output file name.\n";
  print "\t\tProgram will terminate if no file name is provided.\n\n";
  ...
  print "\t-s:\tUse this to specify the lower end of your selected size range.\n";
  print "\t\tProgram will terminate if no value is specified.\n\n"; 
 
  print "\t-S:\tUse this to specify the upper end of your selected size range.\n";
  print "\t\tProgram will terminate if no value is specified.\n\n"; 
...
sub parsecom{ 
  ...
  my $first = $opts{1} or die "\nFirst restriction enzyme (-1) not specified.\n\n";
  my $second = $opts{2} or die "\nSecond restriction enzyme (-2) not specified.\n\n";
  my $file = $opts{f} or die "\nNo input file (-f) specified.\n\n";
  my $outFasta = $opts{o} or die "\nNo output file (-o) specified.\n\n";
  my $lower = $opts{s} or die "\nNo lower size limit (-s) provided.\n\n";
  my $upper = $opts{S} or die "\nNo upper size limit (-S) provided.\n\n";
  
  return( $first, $second, $file, $outFasta, $lower, $upper );
