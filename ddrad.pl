...
#my( $head, $seqs ) = &fastaparse( $file, $seq, $genomelength, $gc, $n );
&fastaparse( \$file, \$genomelength, \$gc, \$n, $min, $max, 
  \$re1xre1select, \$re1xre1discard, 
  \$re2xre2select, \$re2xre2discard, 
  \$re1frags, \$re2frags, 
  \$ddradfrags, \$totalfrags, 
  \$select, \$discard,
  \%hash
  );
...
sub help{
  ...
  print "\nddradsim is a program developed by Steven Michael Mussmann\n\n";
  ...
  print "\t\tFor example, if you have an -s value of 300 and an -e value of 25, the program will calculate the number of fragments between 275bp and 325bp.\n";
...
sub fastaparse{
    my( $file, $genomelength, $gc, $n, $min, $max, $re1xre1select, $re1xre1discard, $re2xre2select, $re2xre2discard, $re1frags, $re2frags, $ddradfrags, $totalfrags, $select, $discard, $hash ) = @_;
    #print Dumper(\%$hash);
	...
	$counter++;
	print "Now working on contig $counter.\n";
	if( $seq ){
	  &cutseq(\$seq, \$genomelength, \$gc, \$n, $min, $max, \$re1xre1select, \$re1xre1discard, \$re2xre2select, \$re2xre2discard, \$re1frags, \$re2frags, \$ddradfrags, \$totalfrags, \$select, \$discard, \$hash );
...
#####################################################################################################
# subroutine to cut a sequence on the restriction sites - designed to be more memory efficient

sub cutseq{

  my( $seq, $genomelength, $gc, $n, $min, $max, $re1xre1select, $re1xre1discard, $re2xre2select, $re2xre2discard, $re1frags, $re2frags, $ddradfrags, $totalfrags, $select, $discard, $hash ) = @_;
  #print Dumper(\%$$hash);
  
  
  # get total length of genome
  $$$genomelength += length( $$seq );
  $$$gc += ( $$seq =~ s/G/G/g );
  $$$gc += ( $$seq =~ s/C/C/g );
  $$$n += ( $$seq =~ s/N/N/g );
